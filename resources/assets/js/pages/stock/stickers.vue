<template>
  <v-container grid-list-xl>
    <v-layout column>
      <v-flex>
        <v-btn color="primary" dark @click.stop="newStickerDialog = true">Новая наклейка</v-btn>
          <v-dialog v-model="newStickerDialog" max-width="500px">
            <v-card>
              <v-card-title>
                <span>
                  Новая наклейка
                ...
                <v-spacer></v-spacer>
                <v-menu bottom left>
                  <v-btn @click="newStickerDialog = false" icon slot="activator">
                    <v-icon>close</v-icon>
                  </v-btn>
                </v-menu>
              </v-card-title>
              <v-card-text>
                <new-sticker-view @new-sticker="onCreate"></new-sticker-view>
              </v-card-text>
            <v-card-actions>
              <v-btn color="primary" flat @click.stop="newStickerDialog=false">Закрыть</v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
      </v-flex>
              ...
              <v-container grid-list-md text-xs-center>
                <v-layout row wrap>
                  ...
                  <v-flex xs12 sm6>
                    ...
                    <v-btn color="primary" dark @click.stop="addStickerDialog = true">Добавить наклеек</v-btn>
                    <!-- <v-btn color="primary" dark @click.stop="stickerToMoneyDialog = true">Перевести в деньги</v-btn> -->
                    <v-dialog v-model="addStickerDialog" max-width="400px">
                      <v-card>
                        <v-card-title>
                          <span>Добавить наклеек</span>
                          <v-spacer></v-spacer>
                          <v-menu bottom left>
                            <v-btn @click="addStickerDialog = false" icon slot="activator">
                              <v-icon>close</v-icon>
                            </v-btn>
                          </v-menu>
                        </v-card-title>
                        <v-card-text>
                          <form>
                            ...
                            ></v-text-field>
                            <v-btn
                              @click="addStickerToRests"
                              >Добавить
                            </v-btn>
                          </form>
                        </v-card-text>
                        <v-card-actions>
                          <v-btn color="primary" flat @click.stop="addStickerDialog=false">Закрыть</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
                  </v-flex>
...
import NewSticker from '~/pages/stock/create/sticker'
...
export default {
  ...
  data () {
      return {
        // dialogs
        newStickerDialog: false,
        addStickerDialog: false,
        stickerToMoneyDialog: false,
  ...
  methods: {
    getStickers() {
      const url = '/api/stock-data/stickers'
    ...
    onCreate() {
      this.newStickerDialog = false;
      this.$store.dispatch('responseMessage', {
          type: 'success',
          text: 'Наклейка создана',
          modal: false
        })
    },

    stickerToMoney() {
      const url = '/api/crud-nomenclatures/manipulations/sticker';
      axios.post(url, {
        sticker_id: this.updateForm.id,
        ...
        this.$store.dispatch('responseMessage', {
          type: 'success',
          text: 'Добавлены остатки по наклейкам',
          modal: false
        })
      })
    },
